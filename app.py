...
if st.session_state.page == "Ion":
    ...
    with t1:
        ...
        if not st.session_state.subscribed:
            ...
            st.markdown('<div class="text-eseu">Un prim episod reprezentativ este cel al <b>horei</b>. Aici este prezentată, în miniatură, întreaga structură socială a satului Pripas. Stratificarea este evidentă: bogații satului stau separat de „sărăntoci”. Ion o alege la joc pe Ana, fata bogătașului Vasile Baciu, reprezentând primul pas dintr-un plan calculat de a obține pământ.</div>', unsafe_allow_html=True)
            ...
            st.markdown('<div class="text-eseu">Romanul se remarcă printr-o <b>structură circulară</b>, bazată pe simetrie. Imaginea drumului care intră în satul Pripas la începutul cărții și drumul care părăsește satul în final sugerează indiferența lumii față de dramele individuale. Compozițional, textul este împărțit în cele două volume menționate anterior.</div>', unsafe_allow_html=True)
            
            st.markdown('<div class="titlu-sectiune">V. Concluzie</div>', unsafe_allow_html=True)
            st.markdown('<div class="text-eseu">În concluzie, prin <span class="highlight">"Ion"</span>, Liviu Rebreanu creează un personaj monumental care eșuează din cauza propriei lăcomii. Opera rămâne o capodoperă a realismului prin rigoarea construcției și prin profunzimea analizei sociale.</div>', unsafe_allow_html=True)
    ...
    with t2:
        if not st.session_state.subscribed:
            ...
            # --- Jocurile 1-7 rămân neschimbate ---
            with st.expander("1. Teorie"):
                if st.selectbox("Anul?", ["1900", "1920"]) == "1920": st.success("+10 pct")
            
            # ... (aici vin restul jocurilor 2-7 din codul anterior) ...

            # --- NIVEL 8 ACTUALIZAT CU IBOVNICĂ ---
            with st.expander("8. Analiză de text (Citat)", expanded=True):
                ...
                if st.button("Verifică Nivel 8"):
                    if r8.lower().strip() in ["ibovnică", "ibovnica"]:
                        st.success("EXACT! 'Ibovnică' este termenul corect folosit de Rebreanu. +25 pct")
                        st.session_state.score += 25
                    elif r8.lower().strip() == "amantă":
                        st.warning("Aproape! În limbaj modern e corect, dar Rebreanu folosește termenul arhaic 'ibovnică'.")
                    else:
                        st.error("Incorect.")

            # --- Jocurile 9-10 rămân neschimbate ---
