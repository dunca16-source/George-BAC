...
if st.session_state.page == "Ion":
    ...
    with t1:
        ...
        if not st.session_state.subscribed:
            st.warning("🔒 Secțiunile III, IV și V (analiza detaliată și simbolurile) sunt blocate. Introdu codul Admin!")
        else:
    ...
    with t2:
        if not st.session_state.subscribed:
            ...
            # Nivel 1: Teorie
            with st.expander("1. Anul Apariției și Curentul Literar", expanded=True):
            ...
            with st.expander("4. Arhitectura Romanului"):
                ...
                if st.button("Verifică Nivel 4"):
                    if structura == "Circulară": st.success("Corect! Simetria drumului. +10 pct"); st.session_state.score += 10
            ...
            # Nivel 6: Simbolul Drumului
            with st.expander("6. Metafora de început"):
            ...
            # Nivel 7: Conflictul principal
            with st.expander("7. Rivalitatea"):
            ...
            # Nivel 8: Citatul celebru
            with st.expander("8. Analiză de text"):
                st.write("'Îl sărută cu patimă, ca pe o ...'")
                cuv = st.text_input("Completează cuvântul lipsă:")
                if st.button("Verifică Nivel 8"):
                    if cuv.lower().strip() == "amantă": st.success("Bravo! Ai reținut citatul cheie. +25 pct"); st.session_state.score += 25

            # Nivel 9: Destinul Anei
            with st.expander("9. Tragismul personajelor"):
            ...
            # Nivel 10: Finalul lui Ion
            with st.expander("10. Deznodământul"):
                final = st.radio("Cum moare protagonistul?", ["Bătrân și bogat", "Ucis de George cu sapa", "Într-un accident"])
...
# --- PAGINA ACASA & BIBLIOTECA (Simple pentru navigare) ---
